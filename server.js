...
function orderText(order,includeActions=false){
 ...
 l.push('','ORDER ITEMS');
 order.items.forEach((i,index)=>{l.push(`${index+1}. ${i.product}${i.arabic ? ' — '+i.arabic : ''}`);l.push(`   SIZE: ${i.weight||'—'} | QTY: ${i.qty||1} | PRICE: $${money(i.lineTotal).toFixed(2)}`);});
 if(order.customer.location && order.customer.location.lat && order.customer.location.lng){const lat=Number(order.customer.location.lat),lng=Number(order.customer.location.lng);l.push('','DELIVERY LOCATION: https://www.google.com/maps?q='+encodeURIComponent(lat+','+lng));}
...
function computeBenefit(customer,baseAmount){const rate=customer.purchases>=7?0.05:customer.purchases>=4?0.04:0.033;return {rate,amount:money(Number(baseAmount||0)*rate),expiresAt:new Date(Date.now()+45*86400000).toISOString()};}
